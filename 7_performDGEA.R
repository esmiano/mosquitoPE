...
library(DESeq2)       # Differential Espression Analysis
library(ggplot2)      # Plotting figures
library(pheatmap)     # Plotting heatmaps
library(ggrepel)      # Non-overlapping plot labels
library(PoiClaClu)    # Poisson similarity matrix
library(ashr)         # Shrinkage algorithm
library(cowplot)      # Plotting legends separately
...
p_pca <- ggplot(pca_data, aes(x = PC1, y = PC2)) +
  ...
  # Stroke colour
  scale_colour_manual(
    # Add previously defined timepoint palette
...
# PLot euclidean distance matrix heatmap
pheatmap(
  sampleDistMatrix,
...
# PLot Poisson distance matrix heatmap
pheatmap(
...
plotExpression <- function(gene_name) {
...
# Identify VOC receptors with at least one significant contrast
sig_voc <- res_wald_df %>%
  filter(gene_id %in% voc_receptors, padj <= 0.05) %>%
  pull(gene_id) %>%
  unique()

# Generate plots without legends
for (gene in c(hygroreceptors, co2_receptors, sig_voc)) {
  ggsave(paste0("expr_", gene, ".png"), plot = plotExpression(gene),
         path = figures_dir, width = 5, height = 4, dpi = 300)
}

# Plot with legend
p_with_legend <- plotExpression("Ir93a") + theme(legend.position = "bottom")

# Save legend separately, without plot
...
# Export CSV
# row.names = FALSE prevents duplication of the gene ID column
write.csv(master_final, file.path(results_dir, "DEG_all_results.csv"), row.names = FALSE)

# Prepare finalised shrunken data.frame for downstream use
shrunk_final <- res_wald_shrunk_df_ord %>%
  # Move contrast column to position after Gene ID
  relocate(contrast, .after = gene_id)
...
write.csv(goi_final, file.path(results_dir, "DEG_GOI_results.csv"),
          row.names = FALSE)
